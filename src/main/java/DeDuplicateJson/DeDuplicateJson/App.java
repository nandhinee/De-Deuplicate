...
public class App 
{
    ...
    public static void main( String[] args )
    {
    	...
    	String inputFilePath = "input\\leads3.json";
		...
		Set<Lead> leadsToPrint = new HashSet<Lead>();
    	if(leadsObj != null && leadsObj.leads.size() > 0) {
    		
			for(Lead curr: leadsObj.leads) {
				// process only if id, email, date are not null
				if(curr._id != null && !curr._id.isEmpty() && curr.email != null && !curr.email.isEmpty() && curr.entryDate != null) {
					if (!set.contains(curr)) {
						addLeadToResult(set, leadsToPrint, curr, null);
					}
					else {
						...
						for(Lead dup: dups) {
							...
							if(dup.entryDate.after(curr.entryDate)) {
								log.info("duplicate encountered but skipping since new lead "+curr._id+"is older than lead "+dup._id);
								continue;
							}
							else {
								try {
								...
								addLeadToResult(set, leadsToPrint, curr, dup);
    	...
    	Leads result = new Leads();
		result.leads = new ArrayList<Lead>(leadsToPrint);
		
		String outputFilePath = "output\\result3.json";
		try {
			processJson.writeToOutput(mapper, outputFilePath, result);
		} catch (Exception e) {
			log.error(e.getStackTrace());
		} 
    }

	private static void logDifferences(Lead prev, Lead curr) {
	...
	private static void addLeadToResult(Set<Lead> set, Set<Lead> leadsToPrint, Lead curr, Lead prev) {
		if(prev != null) {
			set.remove(prev);
